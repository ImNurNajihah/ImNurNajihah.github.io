async function calculateTotalScore() {
    ...
    var total_score = (length_score + complexity_score + entropy_score + predictability_score + keyboard_score + dictionary_score + reuse_score)/7;
...
function calculateBruteForceTime(password, char_set) {
    ...
    console.log(`Est_time (s): ${Est_time.toFixed(4)}`);

    // Round off the estimated time to 2 decimal places
    const roundedEst_time = parseFloat(Est_time.toFixed(4));
...
function updateStrengthBar(total_score) {
    ...
    else if (total_score >= 0.9 && total_score < 3.6) {
        strength_bar.classList.remove('veryweak');
    ...
    else if (total_score >= 3.6 && total_score < 5.4) {
        strength_bar.classList.remove('veryweak');
        strength_bar.classList.remove('weak');
    ...
    else if (total_score >= 5.4 && total_score < 8.1) {
        strength_bar.classList.remove('veryweak');
        strength_bar.classList.remove('weak');
        strength_bar.classList.remove('moderate');
